...
const login = async (loginDetails: ILogin, cookies: any) => {
    try {
        ...
        if (user && isMatched) {
            ...
            const accessToken = createToken({ id: user._id, name: user.name, role: user.roleId }, process.env.ACCESS_TOKEN_SECRET || '', '1d');
            const refreshToken = createToken({ id: user._id }, process.env.REFRESH_TOKEN_SECRET || "", '365d');
